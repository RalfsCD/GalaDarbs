...
<div class="max-w-7xl mx-auto px-2 sm:px-6 py-3 sm:py-6 space-y-4 sm:space-y-6">
    ...
    <div class="w-full max-w-[18rem] sm:max-w-none mx-auto
                p-3 sm:p-6 rounded-2xl bg-white dark:bg-gray-800 shadow-md
                border border-gray-200 dark:border-gray-700 hover:shadow-lg transition space-y-4 sm:space-y-6">
        <form id="updateProfileForm" method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="space-y-4 sm:space-y-6">
            ...
            <div>
                ...
                <label
                    for="profile_photo"
                    class="flex items-center justify-center w-full min-h-[42px] px-3 py-2 rounded-lg
            ...
            <button type="submit"
                class="w-full sm:w-auto inline-flex items-center justify-center gap-2
                       min-h-[42px] px-4 sm:px-5 py-2 rounded-full
    ...
    <div class="w-full max-w-[18rem] sm:max-w-none mx-auto
                p-3 sm:p-6 rounded-2xl bg-white dark:bg-gray-800 shadow-md space-y-3 sm:space-y-4">
        ...
        <button id="deleteAccountBtn"
            class="w-full sm:w-auto inline-flex items-center justify-center gap-2
                   min-h-[42px] px-4 sm:px-5 py-2 rounded-full
                   bg-red-500 text-white font-bold border border-red-400 shadow-md
    ...
    <!-- Profile Deletion / Validation Modal -->
    <div id="validationModal"
         class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 hidden px-2">
        <div class="bg-white dark:bg-gray-800 p-4 sm:p-6 rounded-2xl shadow-lg
                    w-[calc(100%-1.5rem)] max-w-[20rem] sm:max-w-lg space-y-3 sm:space-y-4 relative">
            <button id="closeModal"
                    class="absolute top-2.5 right-2.5 text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white text-2xl leading-none">&times;</button>

            <h2 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-gray-100" id="modalTitle">Notice</h2>
            ...
            <div id="modalSubmit" class="flex justify-end mt-1 hidden">
                <button id="modalConfirmBtn"
                        class="px-4 py-2 rounded-full bg-red-500 text-white font-bold hover:bg-red-600 transition">
                    Confirm
                </button>
            </div>
        </div>
