...
<div class="post-card p-4 sm:p-6 select-none rounded-3xl
  ...
  <div class="flex justify-between items-center mb-4">
    <div class="flex items-center gap-3 min-w-0">
      @if($post->user->profile_photo_path)
        <img src="{{ asset('storage/' . $post->user->profile_photo_path) }}" alt="{{ $post->user->name }}"
        ...
        <p class="text-xs text-gray-600 dark:text-gray-400">
          <a href="{{ route('groups.show', $post->group) }}"
             class="inline-flex items-center gap-1.5 max-w-full
                    bg-yellow-50/80 dark:bg-yellow-500/15 text-yellow-900 dark:text-yellow-100
                    border border-yellow-300/60 dark:border-yellow-500/40
                    rounded-full px-2.5 py-0.5 font-semibold hover:bg-yellow-100/80 dark:hover:bg-yellow-500/25 transition">
            ...
            <span class="truncate max-w-[180px] sm:max-w-[260px]" title="{{ $post->group->name }}">{{ $post->group->name }}</span>
          </a>
        </p>
      </div>
    </div>
    ...
    <div class="relative overflow-hidden rounded-xl mb-4">
      ...
      <button type="button"
        class="expand-image absolute top-2 right-2 z-10 inline-flex items-center justify-center w-9 h-9 sm:w-10 sm:h-10
               ...
               focus:outline-none focus:ring-2 focus:ring-yellow-300" aria-label="Expand image">
  ...
  <div class="flex items-center gap-4 sm:gap-6 mt-3 flex-wrap">
    <button type="button" class="like-btn flex items-center gap-2 group" data-post="{{ $post->id }}" data-like-url="{{ route('posts.like', $post, false) }}">
...
<script>
function openReportModal(id){const el=document.getElementById(`reportModal-${id}`);el.classList.remove("hidden");el.classList.add("flex");}
function closeReportModal(id){const el=document.getElementById(`reportModal-${id}`);el.classList.add("hidden");el.classList.remove("flex");}
</script>
