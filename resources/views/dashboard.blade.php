...
<div class="max-w-7xl mx-auto px-3 sm:px-6 py-6 sm:py-8 space-y-4 sm:space-y-6">
  ...
  <nav aria-label="Breadcrumb"
    ...
    <ol class="flex items-center flex-wrap gap-1.5 text-xs sm:text-sm text-gray-600 dark:text-gray-300">
      ...
      <li class="text-gray-900 dark:text-gray-100 font-semibold">Dashboard</li>
      <li aria-hidden="true" class="text-gray-400">
        <svg class="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
             stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
      </li>
        ...
        <div class="inline-flex p-1 rounded-full bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 shadow-sm">
          <button id="btn-overview"
                  class="mode-btn data-[active=true]:bg-yellow-400 data-[active=true]:text-gray-900
                         data-[active=true]:ring-1 data-[active=true]:ring-yellow-300
                         data-[active=true]:dark:ring-yellow-500
                         h-10 px-4 rounded-full text-sm font-semibold
                         text-gray-700 dark:text-gray-500 transition"
                  data-target="overview" data-active="true" aria-pressed="true">
            Overview
          </button>
          <button id="btn-feed"
                  class="mode-btn data-[active=true]:bg-yellow-400 data-[active=true]:text-gray-900
                         ...
                         h-10 px-4 rounded-full text-sm font-semibold
                         text-gray-700 dark:text-gray-500 transition"
                  data-target="feed" aria-pressed="false">
            Feed
          </button>
        </div>
      </div>
  ...
  <section id="view-overview" class="space-y-6 sm:space-y-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
      ...
      {{-- KPI: Explore Groups (kept; this is separate from the header button) --}}
      <div class="rounded-2xl p-5 h-32 flex flex-col justify-between
                  bg-white/80 dark:bg-gray-900/70 backdrop-blur
                  border border-gray-200/70 dark:border-gray-800/70
                  shadow-[0_12px_32px_-20px_rgba(0,0,0,0.35)]">
    ...
    {{-- ===== Your Groups (preview 3 + Show all / Show less ONLY) ===== --}}
    <section class="space-y-3">
      <div class="flex items-center justify-between">
        <h2 class="text-lg sm:text-xl font-bold text-gray-900 dark:text-gray-100">Your Groups</h2>

        {{-- Only show the toggle if there are more than 3; no other button shown --}}
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        @forelse($groupsPreview as $group)
          <a href="{{ route('groups.show', $group) }}" class="group block">
            <div class="p-5 sm:p-6 rounded-2xl bg-white/80 dark:bg-gray-900/70 backdrop-blur border border-gray-200/70 dark:border-gray-800/70 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition">
      ...
      {{-- Remaining groups (initially hidden) --}}
      @if($groupsRest->count())
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (toggleLessBtn && groupsRest) {
    toggleLessBtn.addEventListener('click', collapseRest);
  }
  ...
  btns.forEach(b => b.addEventListener("click", () => setMode(b.dataset.target)));
  modeLinks.forEach(l => l.addEventListener("click", () => setMode(l.dataset.target)));
