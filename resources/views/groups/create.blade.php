{{-- =============================================================
  groups/create.blade.php — Tailwind-only + plain JS
  - Glass hero + ambient beams
  - NEW: Breadcrumbs (PostPit > Groups > Create)
============================================================= --}}
    ...
    <div class="rounded-3xl bg-white/80 dark:bg-gray-900/70 backdrop-blur border border-gray-200/70 dark:border-gray-800/70 shadow-[0_16px_44px_-22px_rgba(0,0,0,0.45)] p-4 sm:p-6 space-y-6">
      ...
      <div class="space-y-2">
        <div class="flex items-center justify-between">
          ...
          <span id="selected-count" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] sm:text-xs font-semibold bg-yellow-400/15 dark:bg-yellow-500/20 text-yellow-900 dark:text-yellow-100 border border-yellow-300/40 dark:border-yellow-500/40">
            <span class="h-1.5 w-1.5 rounded-full bg-yellow-400"></span>
            0 selected
          </span>
        </div>

        <div id="topicsWrapper" class="rounded-xl border border-gray-200 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-950/30 p-3 sm:p-4">
          <div class="flex flex-wrap gap-2">
            @foreach($topics as $topic)
              <button type="button"
                      class="topic-bubble inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-sm font-medium border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-200 bg-white/80 dark:bg-gray-800/60 hover:-translate-y-0.5 hover:shadow-md transition-all select-none"
                      data-id="{{ $topic->id }}"
                      aria-pressed="false">
                {{ $topic->name }}
              </button>
...
<script>
  ...
  const activeClasses = ['bg-yellow-400','text-gray-900','dark:bg-yellow-500','dark:text-gray-900','border-yellow-300'];
  const defaultClasses = ['bg-white/80','dark:bg-gray-800/60','text-gray-700','dark:text-gray-200','border-gray-300','dark:border-gray-600'];
  ...
  document.getElementById('createGroupForm').addEventListener('submit', function(e) {
    const errors = []; const name = nameInput.value.trim();
    ...
    if (!name) { errors.push('Group name is required.'); nameInput.classList.add('ring-2','ring-red-500','border-red-600'); }
    if (selectedTopics.length === 0) { errors.push('Please select at least one topic.'); topicsWrapper.classList.add('ring-2','ring-red-500','border-red-600'); }
