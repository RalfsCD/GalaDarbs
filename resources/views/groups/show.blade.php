      ...
      <div class="flex items-center gap-2 md:self-start">
        ...
        @if(auth()->id() === $group->creator_id || auth()->user()->isAdmin())
          <form action="{{ route('groups.destroy', $group) }}" method="POST" onsubmit="return confirm('Delete this group?');" class="shrink-0">
            @csrf @method('DELETE')
            <button type="submit" class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-red-500 hover:bg-red-600 text-white transition shadow-sm hover:shadow-md focus:outline-none focus:ring-2 focus:ring-red-300" title="Delete group">
