...
@section('content')
<div class="max-w-7xl mx-auto px-6 py-8 space-y-8"> {{-- Ensuring padding and margins are consistent with groups.index --}}
    
    {{-- Group Info --}}
    <div class="p-6 rounded-2xl bg-white dark:bg-gray-800 shadow-md flex flex-col sm:flex-row justify-between gap-4">
        <div class="space-y-2 sm:space-y-0 sm:w-3/4">
        ...
        <form action="{{ route('groups.destroy', $group) }}" method="POST"
            ...
            @method('DELETE')
            <button type="submit"
                    class="flex items-center justify-center w-10 h-10 rounded-full bg-red-500 hover:bg-red-600 text-white transition">
    ...
    <div class="flex gap-2 mt-4">
        ...
        <form action="{{ route('groups.leave', $group) }}" method="POST">
            @csrf
            <button type="submit"
                    class="px-4 py-2 rounded-full border bg-green-200/80 text-green-900 font-bold hover:bg-green-300 dark:hover:bg-green-600 transition">
                 Joined
            </button>
        </form>
        @else
